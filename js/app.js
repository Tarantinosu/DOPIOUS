...
function chkA(){
  if(document.getElementById('aPw').value==='dopious2026'){
    cAL();
    /* Load admin CSS + JS on demand */
    if(!document.getElementById('admin-css')){
      const link=document.createElement('link');link.id='admin-css';link.rel='stylesheet';link.href='css/admin.css';document.head.appendChild(link);
    }
    if(!document.getElementById('admin-js')){
      const s=document.createElement('script');s.id='admin-js';s.src='js/admin.js';document.head.appendChild(s);
    }else{
      op('aP');rAP();rACl();rATm();
    }
  }else{
...
/* Called by admin.js after it loads */
window._adminReady=function(){op('aP');rAP();rACl();rATm();};
